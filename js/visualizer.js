/**
 * Visualizer Module — Renderiza grafos en un canvas usando layout de fuerzas (force-directed).
 * 
...
const Visualizer = (() => {
     ...
     * ¿Qué recibe?: sx (coordenada X de la pantalla), sy (coordenada Y de la pantalla).
    ...
    /**
     * Función: hexToRgba
     * ¿Qué recibe?: hex (texto como '#FF0000'), alpha (decimal ej: 0.5 para transparencia)
     * ¿Qué hace?: Transforma un color hexadecimal a RGB matemático para que el 
     * Canvas pueda entender e inyectar transparencia (canal Alpha).
     */
    function hexToRgba(hex, alpha) {
        const r = parseInt(hex.slice(1, 3), 16); // Toma caracteres 1 y 2
        const g = parseInt(hex.slice(3, 5), 16); // Toma caracteres 3 y 4
        const b = parseInt(hex.slice(5, 7), 16); // Toma caracteres 5 y 6
    ...
    /**
     * Función: loadGraph
     * ¿Qué recibe?: data (Objeto JSON) - Toda la información de un grafo a mostrar.
     ...
     * ¿Qué recibe?: canvasEl (Elemento HTML), data (Objeto grafo).
